...
export const recupererTousLesLivresBibliotheque =  async(bibliothequeId, afficherTous) =>{
    ...
    if (afficherTous) {
        sqlQuery = `SELECT id, titre, auteur, isbn, description, disponible FROM livres WHERE bibliotheque_id = $1 ORDER BY titre`;
    } else {
        sqlQuery = `SELECT id, titre, auteur, isbn, description, disponible FROM livres WHERE bibliotheque_id = $1 AND disponible = true ORDER BY titre`;
    }
